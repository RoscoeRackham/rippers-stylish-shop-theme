...
// Checks: TWO-ASSET RULE (assets named exactly module.json + rippers-stylish-shop-theme.zip — a missing/mis-named
...
const REPO = 'RoscoeRackham/rippers-stylish-shop-theme';
const ID = 'rippers-stylish-shop-theme';
...
try {
	...
	// bundled register faces: the CSS @font-face rules resolve to local fonts/*.woff2 (so Pirata
	// resolves without rippers-theme) — the zip must actually carry them.
	check(/fonts\/[^\s]+\.woff2/.test(listing), 'bundled fonts/*.woff2 shipped');
	check(listing.includes('presets/rippers-guild.json'), 'preset shipped: presets/rippers-guild.json');
	check(listing.includes('LICENSE'), 'LICENSE shipped in zip');
	check(listing.includes('README.md'), 'README.md shipped in zip');
